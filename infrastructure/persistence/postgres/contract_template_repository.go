...
func (r *ContractTemplateRepository) GetAllContracts(ctx context.Context, contractStatus string, userId int64) ([]model.ContractWithJsonB, error) {
	...
	sqlQuery := "SELECT id, contract_parameters, requisites, manager, type, status, comment  FROM contracts"
	...
	for rows.Next() {
		...
		err := rows.Scan(&i.ID, &i.ContractParameters, &i.Requisites, &i.Manager, &i.Type, &i.Status, i.Comment)
